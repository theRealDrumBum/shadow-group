...
export default async function RosterPage() {
  ...
  return (
    <main className="section command-page">
      ...
      <p>Identity, rank, field role, patch date, public visibility, and long-term event attendance — invitations received, games actually played, and who RSVP&apos;d yes but didn&apos;t show.</p>
      <div className="admin-table">
        <div className="admin-table-head roster-row"><span>CALLSIGN</span><span>NAME</span><span>RANK</span><span>ROLE</span><span>JOINED</span><span>INVITED</span><span>SAID YES</span><span>GAMES</span><span>NO-SHOWS</span><span>PUBLIC</span></div>
        {((operators ?? []) as OperatorRow[]).map((operator) => (
          <div className="admin-table-row roster-row" key={operator.id}>
            ...
            <span>{invited.get(operator.id) ?? 0}</span>
            <span>{going.get(operator.id) ?? 0}</span>
      ...
      <div className="notice">
        Attendance is recorded from event RSVPs: publishing an event invites the whole roster, members reply going /
        not going, and administrators mark who actually attended in the Events module — which builds the invited,
        said-yes, games-played, and no-show history shown here. Only members with a linked operator appear.
      </div>
    </main>
  );
}
