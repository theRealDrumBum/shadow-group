...
type ProfileRow = SessionProfile & {
...
export async function promoteAllowlistedAccount(admin: SupabaseClient, user: User) {
  ...
  const matched = ((allowList ?? []) as AllowListRow[]).find((row) =>
  ...
  const existing = existingRow as ProfileRow | null;
  ...
  if (primaryEmail) {
    ...
    const conflicting = ((others ?? []) as Array<{ id: string; email: string | null }>).filter((row) =>
      emailsEquivalent(row.email, primaryEmail),
    );
